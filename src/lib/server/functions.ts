import { gemini3ProModel } from './gemini'
import { type CountryCode, LocalizationStatus, prisma, StoreStatus } from './prisma'
...
export async function analyzeStore(storeId: string) {
  const store = await prisma.storeInformation.findUnique({
    where: { id: storeId, status: StoreStatus.PENDING },
  })
