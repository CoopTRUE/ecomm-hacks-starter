...
**C. Regulatory Compliance (CRITICAL):**
...
1.  **Localized Text**: Identify all visible text in the image (product names, slogans, labels, warnings).
    -   Provide the original text (if you can infer it) and the new localized text found in the image.
    -   Categorize each text element (e.g., "Product Name", "Tagline", "Regulatory Warning").

2.  **Visual Assets & Marks**: Identify specific graphical elements or regulatory marks present in the image.
    -   List any regulatory symbols (e.g., "CE Mark", "Recycling Loop", "Allergen Icon").
    -   Describe any key visual changes made for localization (e.g., "Background changed to tatami mats", "Color palette shifted to pastel").

Output ONLY valid JSON with the following structure:
{
  "localized_text": [
    { "category": "string", "text_content": "string", "location": "string" }
  ],
  "visual_assets": [
    { "type": "mark|symbol|graphic", "description": "string" }
  ],
  "design_changes": [
    "string"
  ]
